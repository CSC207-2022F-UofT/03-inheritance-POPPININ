...
class BagMain {
    ...
    public static void enhanceBags(Bag[] bags, boolean double_enhance_handbags) {
        for (Bag b : bags) {
            b.enhance();
            // enhance ALL handbags if double_enhance_handbags is true
            if (double_enhance_handbags && b instanceof HandBag) b.enhance();
        }
    }
    ...
    public static int countCrossbodyStraps(Bag[] bags) {
        int totalNumOfStraps = 0;
        for (Bag b: bags) {
            if (b instanceof CrossbodyBag) {
                totalNumOfStraps += ((CrossbodyBag) b).getNumberOfStraps();
            }
        }
        return totalNumOfStraps;
    }
}